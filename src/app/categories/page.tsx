...
export default function CategoriesPage() {
  ...
  const [checkedItems, setCheckedItems] = useState<{ [category: string]: Set<string> }>({})
  ...
  useEffect(() => {
    ...
    const fetchItems = async () => {
      try {
        setLoading(true)
        const res = await fetch(`/api/items/?visa=${encodeURIComponent(visa)}&category=${encodeURIComponent(selectedCategory)}`)
  ...
  const toggleItem = (item: string, sub: string) => {
    if (!selectedCategory) return

    const newCheckedItems = { ...checkedItems }
    const key = `${selectedCategory}::${sub}` // unique key per category + sub

    if (!newCheckedItems[key]) {
      newCheckedItems[key] = new Set()
    }

    const categorySet = new Set(newCheckedItems[key])
    ...
    newCheckedItems[key] = categorySet
    setCheckedItems(newCheckedItems)
  }


const addCustomItem = (subcategory?: string) => {
  const targetSub = subcategory || selectedCategoryForItem
  if (newItemText.trim() && targetSub && selectedCategory) {
    const updatedCustomItems = { ...customItems }
    const key = `${selectedCategory}::${targetSub}`

    if (!updatedCustomItems[key]) {
      updatedCustomItems[key] = []
    }
    updatedCustomItems[key].push(newItemText.trim())
    ...
    newCheckedItems[key].add(newItemText.trim()) // auto-check custom item
    setCheckedItems(newCheckedItems)
    ...
    if (subcategory) {
      ...
      updatedShowAddItemForCategory[subcategory] = false
...
const toggleCustomItem = (item: string, sub: string) => {
  if (!selectedCategory) return

  const key = `${selectedCategory}::${sub}`
  const newCheckedItems = { ...checkedItems }
  ...
  const totalCheckedCount = Object.values(checkedItems).reduce((total, categorySet) => {
    return total + categorySet.size
  }, 0)

  const totalItems = items.reduce((sum, group) => sum + group.items.length, 0)
  const generatePDF = async () => {
    if (totalCheckedCount === 0) return
    ...
    try {
      ...
      pdf.text(`Selected: ${totalCheckedCount} items`, 20, 70)
      ...
      // Group checked items by main category → subcategory
      ...
      for (const [key, set] of Object.entries(checkedItems)) {
        ...
        grouped[category][sub] = Array.from(set)
      }
      ...
      pdf.save(`visa_checklist_${country}_${new Date().toISOString().split('T')[0]}.pdf`)
    } catch (error) {
                  ...
                  <div className="space-y-3">
                    {categories.map((cat) => (
                      <Card
                        key={cat.name}
                        onClick={() => setSelectedCategory(cat.name)}
                        className={`p-4 rounded-xl cursor-pointer transition-all duration-200 border ${selectedCategory === cat.name
                      ...
                      {totalItems > 0 && (
                        <div className="flex items-center gap-2 px-4 py-2 bg-white/10 rounded-full">
                          ...
                          <span className="text-white font-semibold">
                            {totalCheckedCount}/{totalItems}
                          </span>
                        </div>
                      )}
                      <button
                        onClick={generatePDF}
                        disabled={totalCheckedCount === 0 || generatingPdf}
                        className={`flex items-center gap-2 px-4 py-2 rounded-full transition-all duration-200 ${totalCheckedCount > 0 && !generatingPdf
                            ...
                            <div className="space-y-3">
                              {group.items.map((item) => (
                                <motion.div
                                  ...
                                  onClick={() => toggleItem(item, group.sub)}
                                >
                                  {checkedItems[`${selectedCategory}::${group.sub}`]?.has(item) ? (
                                    <CheckCircle2 className="text-green-400 flex-shrink-0" size={20} />
                                  ) : (
                                    <Circle className="text-white/40 flex-shrink-0" size={20} />
                                  )}
                                  <span
                                    className={`text-white ${checkedItems[`${selectedCategory}::${group.sub}`]?.has(item)
...
{customItems[`${selectedCategory}::${group.sub}`] &&
  customItems[`${selectedCategory}::${group.sub}`].map((item, customIdx) => (
    <motion.div
    ...
    >
      <div
        ...
        onClick={() => toggleCustomItem(item, group.sub)}
      >
        {checkedItems[`${selectedCategory}::${group.sub}`]?.has(item) ? (
          <CheckCircle2 className="text-green-400 flex-shrink-0" size={20} />
        ) : (
      ...
      <button
        onClick={() => removeCustomItem(`${selectedCategory}::${group.sub}`, item)}
                                ...
                                >
                                  <div className="flex gap-2">
                                    ...
                                    <button
                                      ...
                                      className={`px-3 py-2 rounded-lg text-sm transition-colors ${newItemText.trim()
              ...
              <div className="space-y-4">
                ...
                <div className="flex gap-3 pt-4">
                  ...
                  <button
                    ...
                    className={`flex-1 px-4 py-2 rounded-lg transition-colors ${newItemText.trim() && selectedCategoryForItem
                      ? 'bg-gradient-to-r from-green-500 to-emerald-600 text-white hover:from-green-600 hover:to-emerald-700'
                      : 'bg-white/10 text-white/50 cursor-not-allowed'
                      }`}
                  >
